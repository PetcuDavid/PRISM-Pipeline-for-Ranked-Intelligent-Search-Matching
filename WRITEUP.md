...
│  Pure Python · 0 LLM calls · eliminates ~60%        │
└──────────────────────┬──────────────────────────────┘
...
doesn't waste LLM calls re-analyzing the same queries.
...
Pure Python. Zero LLM calls. Runs in milliseconds. Typically eliminates 50-70%
...
| Approach | LLM calls for 60 gray zone companies | Consistency |
|---|---|---|
...
avoids sending every company to the LLM. This reduces LLM usage by ~15x at the
...
| Disk cache for query analysis | No repeated LLM calls | Slightly stale if query intent changes |
...
significantly without adding LLM costs.
